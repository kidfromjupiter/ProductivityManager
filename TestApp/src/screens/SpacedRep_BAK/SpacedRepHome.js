...
import { setGAuth, setIsSignedIn, setToken } from "../../redux/GAuthSlice";
...
import { deleteEvent, filterEvents, getEvents } from "../../extras/GAuth";
...
import { listCalendars, addCalendar } from "../../extras/GAuth";
import {
	GoogleSignin,
	statusCodes,
	GoogleSigninButton,
...
import { createUser } from "../../extras/BACKEND";
		...
		e.data.items.forEach(({ start, summary, id, extendedProperties }) => {
			...
			if (d.daysTill() < 0) {
				...
				spacedRepIdArray.forEach((idObj) => {
					...
					// if (!spacedRepEvents.includes(idObj.object)) {
					// 	spacedRepEvents.push(idObj.object);
					// }
				});
			}
		});
			...
			.catch((e) => {
				if (e.response.data.error.status == "UNAUTHENTICATED") {
					GoogleSignin.getTokens().then((e) => {
						...
						createUser(e.idToken).then((e) => console.log(e.data));
					});
